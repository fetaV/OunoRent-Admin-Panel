...
const Typography = () => {
    ...
    const filteredData = users
      .filter(
        (user) =>
          ...
          (user.name &&
            user.name.toString().toLowerCase().includes(lowercasedQuery)) ||
          (user.surname &&
            user.surname.toString().toLowerCase().includes(lowercasedQuery))
      )
  ...
  const handleEditModalOpen = (userId) => {
    ...
    if (userToEdit) {
      const formattedDate = new Date(userToEdit.birthDate)
        .toISOString()
        .split("T")[0];
